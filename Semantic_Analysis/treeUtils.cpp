...
char *expTypeStr(ExpType type, bool isarray, bool isstatic)
{
	...
	{
		case Void:
			typeName = (char *)"type void";  //很有意思，(char *)"type void";这个用法
		break;
		case Integer:
                   ...
                   TreeNode *tree)
{
	if(tree->nodekind == DeclK)
	{
		switch(tree->kind.decl)
		{
			...
			case VarK:
				...
				if(tree->isStatic) tree->varKind = LocalStatic;
	...
	//showAllocation(FILE *out, TreeNode *theNode)
	}
		...
		switch(tree->kind.exp)
		{
			...
			case ConstantK:   
				...
				if(tree->isArray)
					...
					fprintf(listing," of %s",expTypeStr(tree->type,tree->isArray,false));
					fprintf(listing, " [mem: %s loc: %d size: %d]", varKindToStr(tree->varKind), tree->offset, tree->size);}
//					fprintf(listing,"Const %s",tree->attr.name);
				...
				else
					{
						...
						fprintf(listing," of %s",expTypeStr(tree->type,tree->isArray,false));
        					//fprintf(listing," of %s",expTypeStr(tree->type,tree->isArray,tree->isStatic));
						//fprintf(listing, "Const %s of %d", tree->attr.name,tree->attr.type);
    						}
					}
				...
				case Boolean:
					fprintf(listing,"Const %s",tree->attr.name);
					fprintf(listing," of %s",expTypeStr(tree->type,false,false));
				break;
				case UndefinedType:
                                        fprintf(listing,"Const %s",tree->attr.name);
                                break;
			...
			case IdK:
				...
				fprintf(listing," of %s",expTypeStr(tree->type,tree->isArray,tree->isStatic));
				showAllocation(listing, tree);
			//	fprintf(listing, " [mem: %s loc: %d size: %d]", varKindToStr(theNode->varKind), theNode->offset, theNode->size);
			break;
			case OpK:
