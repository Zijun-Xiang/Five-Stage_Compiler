	...
	//IfK, WhileK, ForK, CompoundK, ReturnK, BreakK, RangeK
	 switch(currentNode->kind.stmt){
			...
			treeTraverse(currentNode->child[3],symtab);
                        //foffset = foffset-2;
			//foffset=tempOffset;
			//currentNode->size = temp - foffset;////////////////
			currentNode->size = foffset;
			//currentNode->child[1]->offset+=2;
			//foffset = temp;////////
...
{       
	...
	switch (currentNode->kind.exp) {
        ...
        case CallK: {
	     ...
	     //currentNode->child[0]->nodekind=DeclK;
	     //currentNode->child[0]->kind.decl = ParamK;
 	    TreeNode *paramNode = currentNode->child[0];
	while (paramNode != NULL) {
    		paramNode->nodekind = DeclK;
    		paramNode->kind.decl = ParamK;
		...
		//currentNode->child[0]=funcDef->child[0];
		//currentNode->child[0]->kind.decl=ParamK;
            } else {
        ...
        case IdK: {
            ...
            if (declNode == NULL) {
                ...
                // 从定义节点复制类型和偏移
                currentNode->type = declNode->type;
		//if(strcmp(currentNode->attr.name, "j") == 0 && currentNode->type==Integer)
                //currentNode->offset = declNode->offset+=2;
		//else 
	...
	case '[':  // 数组访问
		...
		currentNode->child[0]->kind.decl = VarK;
