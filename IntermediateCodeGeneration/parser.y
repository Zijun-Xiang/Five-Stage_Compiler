...
%type <tree>  declList    //type用来声明语义值的数据类型，白字是TreeNode类型
%type <tree>  decl       
%type <tree>  varDecl
...
%token   <tinfo>  WHILE
%token   <tinfo>  MIN
%token   <tinfo>  MAX //%token用于声明终结符，tinfo表明MAX的值为TokenData
%token   <tinfo>  FALSE
%token   <tinfo>  FOR
%token   <tinfo>  INT
...
scopedVarDecl : STATIC typeSpec varDeclList ';'     {$$ = $3; setType($3,$2,true);}//$3->varKind = LocalStatic;$$->varKind = LocalStatic;}//$$->varKind = LocalStatic;/* DRBC Note: Be careful! And setType.*/}
                | typeSpec varDeclList ';'             {$$ = $2; setType($2,$1,false);}//$$->varKind = Local;$2->varKind=Local;}//$$->varKind=Local;/* DRBC Note: Be careful! And setType.*/}
              ;
...
varDeclId   : ID                                         {$$ = newDeclNode(VarK,UndefinedType,$1);/* newDeclNode */}
   | ID '[' NUMCONST ']'{$$ = newDeclNode(VarK,UndefinedType,$1); $$ -> isArray = true; $1->nvalue=$3->nvalue+1; $$->size=$3->nvalue+1;}
             ;
...
unmatched  : IF simpleExp THEN stmt                     {$$ = newStmtNode(IfK,$1,$2,$4);/* newStmtNode*/}
		...
		//这里ForK对应着semantic.cpp中currentNode->kind.stmt中的ForK类型
//	     | /* empty */                               {$$ = NULL; $$->size += 2;/* NULL*/}
             ;
...
compoundStmt : '{' localDecls stmtList '}'         {$$ = newStmtNode(CompoundK,$1,$2,$3);$2->nodekind==StmtK;$3->nodekind==StmtK;}   //localDecls用来变量定义，stmtList表达式语句和循环判断语句
             ;

localDecls : localDecls scopedVarDecl              {$$ = addSibling($1,$2);}
             | /* empty */                               {$$ = NULL; /* NULL*/}
             ;			//addSibling($1,$2);它的作用是将多个变量声明连成一个链表形式的结构

stmtList   : stmtList stmt /* 像这种语句列表他们通过 addSibling串联起来 */  {$2 == NULL ? $$ = $1 : $$ = addSibling($1,$2);/* addSibling DRBC Note: $2 might legitamately be empty here. If so $$ = $1;*/}
             | /* empty */                               {$$ = NULL; /* NULL*/}
           ;
...
unaryExp   : unaryop unaryExp  {$$ = newExpNode(OpK,$1,$2);
				...
				if ( $1->tokenclass == SIZEOF && $2->kind.exp == IdK){
					$2->isArray = true;              // 设置为目标变量的 size
...
unaryop    : '-'                             {$1->tokenclass=CHSIGN; $$=$1;} //change sign
...
mutable    :ID '[' exp ']' {	
				$$ = newExpNode(OpK,$2,NULL,$3); //This creates an OpK node for the array access ([)
				$$->child[0] = newExpNode(IdK,$1); 
				$$->child[0]->isArray = true;$3->isArray = false;
				$$->child[0]->type = Char;
           ...
           | ID                                       {$$ = newExpNode(IdK,$1); 
							...
							$$->attr.name = $1->svalue; }
	   ;
immutable  : '(' exp ')'                            {$$ = $2;$$->varKind = LocalStatic;/* DRBC Note: Be careful!*/}
             | call				    {$$ = $1;}
             | constant				    {$$ = $1;}
           ;

call       : ID '(' args ')' {$$ = newExpNode(CallK,$1,$3);/*$3->kind.decl=ParamK; $$->isArray = true; */}
           ;
...
argList    : argList ',' exp                       {if ($3->kind.exp == IdK) $3->isArray = true;addSibling($1,$3);}
             | exp					{if ($1->kind.exp == IdK) $1->isArray = true;$$ = $1;}
           ;
...
constant   : NUMCONST                     {$$ = newExpNode(ConstantK,$1); $$->type=Integer; $$ -> attr.value=$1->nvalue;}
 ...
 | STRINGCONST  {$$ = newExpNode(ConstantK,$1); $$->attr.string = strdup($1->svalue); $$->size = $1->nvalue + 1; $$->isArray = true; setType($$,Char,true);}
